...
const TargetScoreSelectionPhase: React.FC<TargetScoreSelectionPhaseProps> = ({ onGoalsSet, practiceMode }) => {
  const [targetScore, setTargetScore] = useState<number>(6.5);
  // Language defaults to 'en'. For mock test, this will be the feedback language.
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-slate-200">
      <div className="p-8 text-center border-b border-slate-200">
        <h3 className="text-2xl font-extrabold text-slate-900">Set Your Practice Goals</h3>
        <p className="mt-2 text-slate-500">
          {isMockTest 
            ? "Set your target score for this mock test."
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8 flex flex-col items-center">
        ...
        <div className="w-full max-w-lg">
           <label htmlFor="score-slider" className="block text-center text-lg font-extrabold text-slate-700 mb-2">
            {isMockTest ? "What's your target band score?" : "1. What's your target band score?"}
          </label>
        ...
        {/* Language Selection Section - Conditional */}
        {!isMockTest && (
          <div className="w-full max-w-lg pt-4 border-t border-slate-200">
            <label className="block text-center text-lg font-extrabold text-slate-700 mb-4">
        ...
        <div className="pt-4">
          <Button type="submit" variant="primary" className="px-8 py-3 text-lg">
            {isMockTest ? "Continue to Time Selection" : "Start AI-Guided Prep"}
          </Button>
        </div>
      </form>
    </div>
